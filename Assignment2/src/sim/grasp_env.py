...
from src.constants import DEPTH_IMG_SCALE, TABLE_HEIGHT, CALIB_GRID_SCALE, CALIB_CHESSBOARD_SIZE
...
class GraspEnvConfig:
    ...
    chessboard_pose: Optional[np.ndarray] = None
...
class GraspEnv:
    ...
    def launch(self):
        ...
        if self.config.chessboard_pose is not None:
            self.chessboard = get_chessboard(self.config.chessboard_pose)
            scene.obj_list.append(self.chessboard)
    ...
    def reset(self, qpos: Optional[np.ndarray] = None):
        ...
        if qpos is None:
            qpos = self.robot_cfg.joint_init_qpos.copy()
        self.sim.reset(qpos)
        _qpos = qpos[:8]
    ...
    def get_obs(self) -> Obs:
        ...
        cam_trans, cam_rot = self.robot_model.fk_camera(self.sim.robot_qpos)
    ...
    def save_obs(self, obs: Obs, data_dir=None):
        ...
        if obs.object_pose is not None:
            np.save(os.path.join(data_dir, "object_pose.npy"), obs.object_pose)

    def set_eef_pose(self, pose: np.ndarray) -> bool:
        """Try to set the end-effector pose. Return False if the pose is not reachable."""
        succ, qpos = self.robot_model.ik(pose[:3, 3], pose[:3, :3])
        if not succ:
            return False
        qpos = np.concatenate([qpos, np.zeros(len(self.robot_cfg.joint_gripper_indices))])
        self.reset(qpos)
        # print(qpos, self.sim.robot_qpos)
        return True
...
def get_chessboard(pose: np.ndarray) -> Box:
    chessboard = Box(
        name="chessboard",
        pose=pose,
        size=np.array([
            # grid num for each row / column = number of inner corners for each row / column + 1
            (CALIB_CHESSBOARD_SIZE[0] + 1) * CALIB_GRID_SCALE, 
            (CALIB_CHESSBOARD_SIZE[1] + 1) * CALIB_GRID_SCALE, 
            0.0001
        ]),
        fixed_body=True,
        material="chess_mat",
