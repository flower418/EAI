...
class MjSim:
    ...
    def render(self, render_cfg=None):
        ...
        camera_name = getattr(render_cfg, "camera_name", "cam0")
        cam_mount_name = getattr(render_cfg, "cam_mount_name", "cam_mount")

        # Use the fixed camera defined in MJCF, not MovableCamera.
        renderer = Camera(
            self.physics,
        ...
        # For fixed cameras, fovy is camera-specific.
        if getattr(render_cfg, "fovy", None) is not None:
            cam_id = self.physics.model.name2id(camera_name, "camera")
            self.physics.model.cam_fovy[cam_id] = render_cfg.fovy  # degrees

        # Drive cam_mount mocap body from extrinsics.
        if getattr(render_cfg, "extrinsics", None) is not None:
            cam_pos_w = render_cfg.extrinsics[:3, 3]
            # we use left, up, forward for extrinsics
            # mujoco use right, up, back
            # so we need to multiply by -1 in x and z
            align_rot = np.array([
                [-1, 0, 0],
                [0, 1, 0],
                [0, 0, -1.]
            ])
            cam_quat_wxyz = mat2quat(render_cfg.extrinsics[:3, :3] @ align_rot)

            body_id = self.physics.model.name2id(cam_mount_name, "body")
            mocap_id = self.physics.model.body_mocapid[body_id]
            if mocap_id < 0:
                raise ValueError(
                    f"Body {cam_mount_name!r} is not a mocap body."
                )

            self.physics.data.mocap_pos[mocap_id][:] = cam_pos_w
            self.physics.data.mocap_quat[mocap_id][:] = cam_quat_wxyz

            # Recompute kinematics without stepping the simulation.
    ...
    def _add_geom_primitive(self, body, obj: Box):
        ...
        if isinstance(obj, Box):
            ...
            if obj.material is not None:
                geom_prop["material"] = obj.material
        else:
